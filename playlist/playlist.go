...
func IsYTDL(path string) bool {
	...
	host = strings.TrimPrefix(host, "www.")
	host = strings.TrimPrefix(host, "m.")
	// Yandex Music URLs are resolved directly by the provider, not via yt-dlp.
	if IsYandexMusicURL(path) {
		return false
	}
	switch host {
	case "soundcloud.com",
...
// IsYandexMusicURL reports whether the URL points to Yandex Music (music.yandex.ru).
func IsYandexMusicURL(path string) bool {
	if !IsURL(path) {
		return false
	}
	u, err := url.Parse(path)
