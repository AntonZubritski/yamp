...
func ParseFlags(args []string) (action string, ov Overrides, positional []string, err error) {
	...
	for i < len(args) {
		...
		case "--provider":
			...
			v = strings.ToLower(v)
			switch v {
			case "radio", "navidrome", "plex", "spotify", "yandex", "yt", "youtube", "ytmusic":
			default:
				return "", ov, nil, fmt.Errorf("flag --provider value must be radio, navidrome, plex, spotify, yandex, yt, youtube, or ytmusic (got %q)", v)
			}
			ov.Provider = &v
		case "--volume":
