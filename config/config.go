// Package config handles loading user configuration from ~/.config/yamp/config.toml.
package config
...
type YandexConfig struct {
	Token string // Yandex OAuth token
}

// IsSet reports whether the Yandex Music token is present.
func (y YandexConfig) IsSet() bool {
	return y.Token != ""
}
...
type Config struct {
	...
	Yandex            YandexConfig       // optional Yandex Music provider
}
...
// Load reads the config file from ~/.config/yamp/config.toml.
...
func Load() (Config, error) {
	...
	for scanner.Scan() {
		...
		case "yandex":
			switch key {
			case "token":
				cfg.Yandex.Token = strings.Trim(val, `"'`)
			}
		default:
			switch key {
			case "volume":
...
// SaveYandexToken writes the Yandex Music OAuth token into the [yandex]
// section of config.toml, creating or updating it as needed.
func SaveYandexToken(token string) error {
	...
	line := fmt.Sprintf("token = %q", token)
	...
	if err != nil {
		...
		return os.WriteFile(path, []byte("[yandex]\n"+line+"\n"), 0o644)
	}

	lines := strings.Split(string(data), "\n")

	// Try to replace an existing token inside [yandex].
	inYandex := false
	for i, l := range lines {
		trimmed := strings.TrimSpace(l)
		if strings.HasPrefix(trimmed, "[") && strings.HasSuffix(trimmed, "]") {
			inYandex = strings.ToLower(trimmed[1:len(trimmed)-1]) == "yandex"
			continue
		}
		if inYandex {
			k, _, ok := strings.Cut(trimmed, "=")
			if ok && strings.TrimSpace(k) == "token" {
				lines[i] = line
				return os.WriteFile(path, []byte(strings.Join(lines, "\n")), 0o644)
			}
		}
	...
	// Key not found: append after the last line in the [yandex] section,
	// or append a new [yandex] section at the end.
	inYandex = false
	insertAt := -1
	for i, l := range lines {
		trimmed := strings.TrimSpace(l)
		if strings.HasPrefix(trimmed, "[") && strings.HasSuffix(trimmed, "]") {
			if inYandex && insertAt >= 0 {
				break
			}
			inYandex = strings.ToLower(trimmed[1:len(trimmed)-1]) == "yandex"
		}
	...
	if insertAt >= 0 {
		...
		lines = append(lines, "[yandex]", line)
	}
