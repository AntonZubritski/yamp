...
Yamp exposes an [MPRIS2](https://specifications.freedesktop.org/mpris-spec/latest/) D-Bus service on Linux. This allows desktop environments, media key daemons, and command line tools like `playerctl` to control playback, read track metadata, and adjust volume without touching the TUI.
...
Yamp registers itself as:

```
org.mpris.MediaPlayer2.yamp
```

Only one instance can hold this name at a time. If a second Yamp process tries to start, the MPRIS registration will fail silently and that instance will run without D-Bus integration.
...
Volume is exposed as a linear value between 0.0 and 1.0. Internally Yamp uses a decibel scale (from 30 dB to +6 dB), and the conversion happens automatically.
...
The `HasTrackList` property is set to false. Yamp does not implement the optional `org.mpris.MediaPlayer2.TrackList` interface.
