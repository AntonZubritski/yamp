...
yamp can play audio from URLs, M3U/PLS playlists, and podcast RSS feeds.
...
yamp http://radio-station.com/stream.m3u
yamp local.mp3 https://example.com/remote.mp3   # mix local + remote
```
...
```sh
yamp https://radio.yamp.stream/lofi/stream.pls
```
...
```sh
yamp https://example.com/podcast/feed.xml
```
...
```sh
yamp https://www.xiaoyuzhoufm.com/episode/xxxx
```
