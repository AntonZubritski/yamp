...
Yamp can connect to a [Navidrome](https://www.navidrome.org/) server and stream music directly from your library. Navidrome is a self-hosted music server compatible with the Subsonic API.

## Setup

Set three environment variables before launching Yamp:
...
Then run Yamp without any file arguments:
...
```sh
NAVIDROME_URL=http://localhost:4533 NAVIDROME_USER=admin NAVIDROME_PASS=secret yamp ~/Music/extra.mp3
```

## How It Works

When the environment variables are set, Yamp authenticates with your Navidrome server using the Subsonic API. On launch it fetches your playlists and presents them in the TUI.
...
The chosen sort is saved automatically to `~/.config/yamp/config.toml` under the `[navidrome]` section as `browse_sort` and is restored on the next launch.
