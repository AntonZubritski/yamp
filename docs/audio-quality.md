...
yamp lets you tune the output sample rate, speaker buffer size, resample quality, and bit depth via `~/.config/yamp/config.toml`. The active settings are shown in the `OUT` status line below the EQ.
