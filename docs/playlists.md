...
Yamp supports two kinds of playlists: **M3U files** loaded from the command line and **local TOML playlists** managed from within the app.
...
```sh
yamp ~/radio-stations.m3u
yamp http://radio.example.com/streams.m3u
yamp ~/music.m3u https://example.com/live.m3u   # mix local + remote
```
...
Create and manage your own playlists stored as `.toml` files in `~/.config/yamp/playlists/`.
...
```toml
# ~/.config/yamp/playlists/radio-stations.toml
...
Running `yamp` without arguments connects to the built-in radio channel. If Navidrome is configured, it opens the provider browser instead.
...
```sh
yamp song.mp3                    # starts playing, Esc opens browser
```
...
The directory `~/.config/yamp/playlists/` is created automatically on first use. Removing the last track from a playlist auto-deletes the file.
...
```sh
mkdir -p ~/.config/yamp/playlists
```

```toml
# ~/.config/yamp/playlists/favorites.toml
