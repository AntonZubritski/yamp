...
```sh
mkdir -p ~/.config/yamp
cp config.toml.example ~/.config/yamp/config.toml
```
...
# UI theme name (see available themes in ~/.config/yamp/themes/)
theme = "Tokyo Night"
...
You can also override from the CLI: `yamp --provider navidrome`.

## Custom Radio Stations

Add your own stations to `~/.config/yamp/radios.toml`:
...
These appear alongside the built-in yamp radio in the Radio provider.

See [audio-quality.md](audio-quality.md) for sample rate, buffer, bit depth, and resample quality settings.

## WSL2 (Windows Subsystem for Linux)

yamp uses ALSA for audio on Linux. WSL2 doesn't expose ALSA hardware directly, but WSLg provides a PulseAudio server that ALSA can route through.
