...
Yamp can stream your [Spotify](https://www.spotify.com/) library directly through its audio pipeline — EQ, visualizer, and all effects apply. Requires a [Spotify Premium](https://www.spotify.com/premium/) account.
...
2. Click **Create app**
3. Fill in a name (e.g. "yamp") and description (anything works)
...
Add your client ID to `~/.config/yamp/config.toml`:
...
Run `yamp`, select Spotify as a provider, and press Enter to sign in. Credentials are cached at `~/.config/yamp/spotify_credentials.json` — subsequent launches refresh silently.
...
Your Spotify playlists are listed in the provider panel. Navigate with the arrow keys and press `Enter` to load one. Tracks are streamed through yamp's audio pipeline, so EQ, visualizer, mono, and all other effects work exactly as with local files.
...
- **Re-authenticate** — Delete `~/.config/yamp/spotify_credentials.json` and restart yamp to trigger a fresh login.

## Requirements

- Spotify Premium account
- A registered app at [developer.spotify.com/dashboard](https://developer.spotify.com/dashboard)
- No additional system dependencies beyond yamp itself
