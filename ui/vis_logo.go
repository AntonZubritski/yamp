...
// logoGlyphs holds 5×7 pixel bitmaps for each letter in "YAMP".
// Each row is 5 bits wide; bit 4 (0x10) is the leftmost pixel.
var logoGlyphs = [4][7]uint8{
...
// renderLogo draws "YAMP" in pixel art using Braille dots. Individual dots
...
func (v *Visualizer) renderLogo(bands [numBands]float64) string {
	...
	// Map 4 letters across the 10 frequency bands.
	letterBand := [4]int{0, 3, 6, 9}
