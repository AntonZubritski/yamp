...
const (
	...
	VisZubritski              // ZUBRITSKI text with EQ bars inside
	VisCustom                 // user-created text visualizer
...
type Visualizer struct {
	...
	// Custom user-created text visualizers.
	CustomConfigs []CustomVisConfig
	CustomIdx     int // active custom visualizer index
}
...
// Skips VisCustom if no custom visualizers are saved.
func (v *Visualizer) CycleMode() {
	v.Mode = (v.Mode + 1) % visCount
	if v.Mode == VisCustom && len(v.CustomConfigs) == 0 {
		v.Mode = (v.Mode + 1) % visCount
	}
}

// CycleCustom cycles through user-created custom visualizers.
func (v *Visualizer) CycleCustom() {
	if len(v.CustomConfigs) == 0 {
		return
	}
	v.CustomIdx = (v.CustomIdx + 1) % len(v.CustomConfigs)
}

// renderCustom renders the active custom text visualizer.
func (v *Visualizer) renderCustom(bands [numBands]float64) string {
	if len(v.CustomConfigs) == 0 {
		return ""
	}
	cfg := v.CustomConfigs[v.CustomIdx%len(v.CustomConfigs)]
	return v.renderCustomText(cfg.Text, effectFromString(cfg.Effect), bands)
}

// LoadCustomConfigs loads saved custom visualizers from disk.
func (v *Visualizer) LoadCustomConfigs() {
	v.CustomConfigs = loadCustomVis()
}
...
var visModes = [visCount]visEntry{
	...
	VisZubritski:       {"Zubritski", (*Visualizer).renderZubritski},
	VisCustom:          {"Custom", (*Visualizer).renderCustom},
	VisNone:            {"None", nil},
}
