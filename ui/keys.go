...
func (m *Model) handleKey(msg tea.KeyMsg) tea.Cmd {
	...
	// Custom visualizer editor overlay
	if m.customVis.visible {
		return m.handleCustomVisKey(msg)
	}
	...
	if m.focus == focusProvider {
		// Yandex token text input mode — capture all keys.
		if m.yandexTokenInput {
			return m.handleYandexTokenKey(msg)
		}
		...
		case "enter":
			if m.provSignIn {
				if m.yandexProv != nil && m.yandexProv.NeedsAuth() {
					// Open browser and switch to token input mode
					_ = m.yandexProv.Authenticate()
					m.yandexTokenInput = true
					m.yandexTokenBuf = ""
					return nil
				}
	...
	case "ctrl+e":
		m.openCustomVisOverlay()
	}

	return nil
}

// saveTrack copies the current track to ~/Music/yamp/ with a clean filename.
...
func (m *Model) saveTrack() tea.Cmd {
	...
	saveDir := filepath.Join(home, "Music", "yamp")
	...
	// YouTube/yt-dlp tracks: async download directly to ~/Music/yamp/.
	...
	m.status.text = fmt.Sprintf("Saved to ~/Music/yamp/%s", name+ext)
	m.status.ttl = statusTTLDefault
	return nil
}
...
// handleYandexTokenKey processes key presses while entering Yandex OAuth token.
func (m *Model) handleYandexTokenKey(msg tea.KeyMsg) tea.Cmd {
	switch msg.Type {
	case tea.KeyEscape:
	...
	case tea.KeyEnter:
		...
		token := strings.TrimSpace(m.yandexTokenBuf)
		m.yandexTokenBuf = ""
		m.yandexTokenInput = false
		m.provSignIn = false
		m.provLoading = true
		return func() tea.Msg {
			if err := m.yandexProv.SetToken(token); err != nil {
				return provAuthDoneMsg{err: err}
			}
			return provAuthDoneMsg{}
		}
	case tea.KeyBackspace:
		if m.yandexTokenBuf != "" {
			m.yandexTokenBuf = removeLastRune(m.yandexTokenBuf)
		}
	case tea.KeyCtrlV:
		// Ctrl+V: read clipboard via PowerShell on Windows, pbpaste on macOS, xclip on Linux.
		return func() tea.Msg {
			return yandexClipboardMsg(readClipboard())
		}
	case tea.KeyRunes:
		// Filter out non-printable and control characters that can leak through.
		for _, r := range msg.Runes {
			if r >= 0x20 && r != 0x7f {
				m.yandexTokenBuf += string(r)
			}
		}
	}
	return nil
...
// yandexClipboardMsg carries clipboard text pasted via Ctrl+V.
type yandexClipboardMsg string
