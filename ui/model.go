// Package ui implements the Bubbletea TUI for the YAMP terminal music player.
package ui
...
// Model is the Bubbletea model for the YAMP TUI.
type Model struct {
	// Core playback
	...
	// yandexProv is kept for token input flow.
	yandexProv       *yandex.Provider
	yandexTokenBuf   string // token being typed during sign-in
	yandexTokenInput bool   // true when the token text field is active

	// urlResolver resolves provider-specific URLs (e.g. Yandex Music) to direct
	// stream URLs before playback. If nil or returns empty string, the original
	// URL is used as-is.
	urlResolver func(trackPath string) (string, error)

	// radioStationID is set when playing a Yandex radio station (e.g. "user:onyourwave").
	// When non-empty, reaching the end of the playlist triggers automatic fetching
	// of the next batch of radio tracks.
	radioStationID string
	// radioTrackLoader fetches the next batch of radio tracks for the given station ID.
	// Set together with urlResolver from main.go.
	radioTrackLoader func(stationID string) ([]playlist.Track, error)
}
...
func NewModel(p *player.Player, pl *playlist.Playlist, providers []ProviderEntry, defaultProvider string, localProv *local.Provider, themes []theme.Theme, navCfg config.NavidromeConfig, nav *navidrome.NavidromeClient) Model {
	...
	// Load custom visualizers.
	m.vis.LoadCustomConfigs()
	return m
}
...
// SetYandexProvider stores a reference to the Yandex provider for token input flow.
func (m *Model) SetYandexProvider(p *yandex.Provider) { m.yandexProv = p }

// SetURLResolver sets a callback that resolves provider-specific URLs
// (e.g. Yandex Music track URLs) to direct stream URLs before playback.
func (m *Model) SetURLResolver(fn func(string) (string, error)) { m.urlResolver = fn }

// SetRadioTrackLoader sets a callback that fetches more tracks for a radio station.
func (m *Model) SetRadioTrackLoader(fn func(string) ([]playlist.Track, error)) {
	m.radioTrackLoader = fn
}
...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tracksLoadedMsg:
		...
		m.provLoading = false
		// Remember radio station for auto-refill.
		if strings.HasPrefix(msg.playlistID, "radio:") {
			m.radioStationID = strings.TrimPrefix(msg.playlistID, "radio:")
		} else {
			m.radioStationID = ""
		}
	...
	case radioRefillMsg:
		...
		// Append new tracks and start playing the first one.
	...
	case yandexClipboardMsg:
		if m.yandexTokenInput {
			text := strings.TrimSpace(string(msg))
			if text != "" {
				m.yandexTokenBuf = text
			}
		}
		return m, nil
...
// If the playlist ends and we're in radio mode, fetches more tracks automatically.
func (m *Model) nextTrack() tea.Cmd {
	track, ok := m.playlist.Next()
	if !ok {
		// Radio auto-refill: fetch next batch and continue playing.
		if m.radioStationID != "" && m.radioTrackLoader != nil {
			m.buffering = true
			m.bufferingAt = time.Now()
			stationID := m.radioStationID
			loader := m.radioTrackLoader
			return func() tea.Msg {
				tracks, err := loader(stationID)
				return radioRefillMsg{tracks: tracks, err: err}
			}
		}
		m.player.Stop()
		return nil
...
func (m *Model) playTrack(track playlist.Track) tea.Cmd {
	...
	// Resolve provider-specific URLs (e.g. Yandex Music) to direct stream URLs.
	if m.urlResolver != nil && playlist.IsYandexMusicURL(track.Path) {
		m.buffering = true
		m.bufferingAt = time.Now()
		m.err = nil
		...
		resolveCmd := func() tea.Msg {
			directURL, err := resolver(path)
			if err != nil {
				return streamPlayedMsg{err: err}
			}
			return streamPlayedMsg{err: m.player.Play(directURL, dur)}
		}
		if fetchCmd != nil {
			return tea.Batch(resolveCmd, fetchCmd)
		}
		return resolveCmd
	}
...
func (m *Model) preloadNext() tea.Cmd {
	...
	// Preload Yandex Music tracks: resolve direct URL, then preload as stream.
	if m.urlResolver != nil && playlist.IsYandexMusicURL(next.Path) {
		...
		return func() tea.Msg {
			...
			m.player.Preload(directURL, nextDur)
			return streamPreloadedMsg{}
		}
	}
