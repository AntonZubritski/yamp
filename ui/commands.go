...
type tracksLoadedMsg struct {
	...
	playlistID string // original playlist ID (used to detect radio stations)
}

// radioRefillMsg carries a batch of new tracks fetched from a radio station.
type radioRefillMsg struct {
	tracks []playlist.Track
	err    error
}
...
func fetchTracksCmd(prov playlist.Provider, playlistID string) tea.Cmd {
	return func() tea.Msg {
		...
		return tracksLoadedMsg{tracks: tracks, playlistID: playlistID}
	}
}
